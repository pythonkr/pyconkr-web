...
const BusNumbers = ["5-3", "11", "19", "20", "20-1", "20-2", "32-4", "201", "670", "720-3", "999"];
      ...
      <div style={{ width: "100%", maxWidth: "1200px" }}>
        ...
        <div>
          <div style={{ ...flexBoxStyle }}>
            <MapSelectTabBtn
              className={state.mapType === "kakao" ? "active" : ""}
              onClick={() => selectMap("kakao")}
            >
              {t("카카오맵")}
            </MapSelectTabBtn>
            <MapSelectTabBtn
              className={state.mapType === "google" ? "active" : ""}
              onClick={() => selectMap("google")}
            >
              {t("구글 지도")}
            </MapSelectTabBtn>
          </div>
          ...
          <div
            style={{ ...columnFlexBoxStyle, display: state.mapType === "kakao" ? "flex" : "none" }}
          >
            <div ref={kakaoMapRef} style={mapStyle}></div>
            <FullWidthAnchorBtn href={KAKAO_MAP_URL}>
              <button className="kakao">{t("카카오맵에서 열기")}</button>
            </FullWidthAnchorBtn>
            <FullWidthAnchorBtn href={NAVER_MAP_URL}>
              <button className="naver">{t("네이버 지도에서 열기")}</button>
            </FullWidthAnchorBtn>
          </div>
          <div
            style={{ ...columnFlexBoxStyle, display: state.mapType === "google" ? "flex" : "none" }}
          >
            <iframe
              title="map"
              src={GOOGLE_MAP_IFRAME_SRC}
              style={mapStyle}
              allowFullScreen
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
            ></iframe>
            <FullWidthAnchorBtn href={GOOGLE_MAP_URL}>
              <button className="google">{t("구글 지도에서 열기")}</button>
            </FullWidthAnchorBtn>
          </div>
          <br />
          <ul>
        ...
        <TransportInfoDetails open>
          ...
          <div>
            ...
            />
            <div>
              <kbd className="subway-gwanggyojungang">{t("신분당선 「광교중앙역」")}</kbd>{" "}
              {t("4번 출구에서 도보 10분")}
            </div>
          </div>
        ...
        <TransportInfoDetails>
          ...
          <div>
            ...
            {t("서울역")} <kbd className="widearea-express">M5115</kbd>
            <kbd className="widearea-express">M5121</kbd> / {t("강남역")}{" "}
            <kbd className="widearea-express">M5422</kbd>
            <br />→ {t("「광교중앙·아주대환승센터」 하차")}
            <br />→ <kbd className="subway-gwanggyojungang">
              {t("신분당선 「광교중앙역」")}
            </kbd>{" "}
            {t("4번 출구에서 도보 10분")}
            <hr />
            ...
            {BusNumbers.map((v, i) => (
              <kbd className="city" key={i}>
                {v}
              </kbd>
            ))}
            <br />→ {t("수원컨벤션센터·롯데아울렛 하차")}
          </div>
        </TransportInfoDetails>
        <TransportInfoDetails>
          ...
          <div>
            ...
            <kbd style={{ backgroundColor: "rgb(131, 165, 206)" }}>8877</kbd> {t("리무진 버스")}{" "}
            (60~80{t("분")})<br />→ {t("수원컨벤션센터·광교갤러리아 백화점 하차")}
            <AirportBusDetails>
                ...
                <table>
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        {t("제2터미널")} ↔ {t("제1터미널")} ↔{" "}
                        <b>{t("수원컨벤션센터·광교갤러리아 백화점")}</b> ↔{" "}
                        {t("흥덕지구입구 ↔ 신갈(용인) ↔ 한국민속촌")}
                      </td>
                    </tr>
                    <tr>
                      <td>{t("요금")}</td>
            ...
            <hr />
            <h5>{t("김포공항 「동수원」행")}</h5>
            <kbd style={{ backgroundColor: "rgba(211, 0, 2, 1)", color: "#fff" }}>4300</kbd>{" "}
            {t("시외버스")} (80{t("분")})<br />→ {t("동수원 하차")}
            <br />→ {t("(택시 10분 또는 버스 30분)")}
            <AirportBusDetails>
              <summary>{t("노선 세부정보")}</summary>
              <div>
                <table>
        ...
        <ul>
          ...
          <li>
            <b>{t("카드결제만 가능합니다. (신용/체크/교통/삼성페이)")}</b>
          </li>
          <li>
            {t(
          ...
          <li>
            {t("주차요금 감면 등의 자세한 내용은")}{" "}
            <a href={t("https://www.scc.or.kr/parking-2/")} style={{ color: "rgb(254, 189, 153)" }}>
              {t("수원컨벤션센터 주차안내 페이지")}
            </a>
            {t("를 참고해주세요.")}
          </li>
